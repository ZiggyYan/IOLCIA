...
def _make_dataset(dir):
    images = []
    # lesion_adr = ['benign','malignant','normal']
...
class busi(data.Dataset):
    ...
    def __init__(self, root, joint_transform=None,
        ...
        self.imgs = _make_dataset(os.path.join(self.root))

    def __getitem__(self, index):
        # time.sleep(0.5)
        path = self.imgs[index]
        img = self.loader(path)
        t_path = path.replace('image', 'mask')
        t_path = t_path.replace('.png', '_mask.png')
        if 'normal' in path:
            target = Image.new('1', (img.size[0],img.size[1]))
        else:
