...
from Seg_Module.modeling.deeplab_dense import *
# import utils.imgs
# import utils.training as train_utils
from Loader.BUSI import busi
from Loader import joint_transforms
...
train_path = '../Data/BreastUltrasound/BUSI/train/image/'
test_path = '../Data/BreastUltrasound/BUSI/test/image/'
RESULTS_PATH = './models/GlobalGuidance_Net/Results/'
WEIGHTS_PATH = './models/GlobalGuidance_Net/ultra_weights/dense1'



batch_size = 4

ckpt_path = './models/GlobalGuidance_Net/Results/'
args = {
    'iter_num': 30000,
    'train_batch_size': 10,
    'lr': 1e-3,
...
e_loss=edge_loss()
d_loss=DiceLoss()
log_path = os.path.join(ckpt_path, str(datetime.datetime.now()))
...
def main():
  ...
  # train_set = busi(train_path, joint_transforms, transform, target_transform)
  # test_set= busi(test_path, None, val_transform, val_target_transform)
  train_set = busi(train_path, joint_transforms, transform, target_transform)
  test_set= busi(test_path, None, val_transform, val_target_transform)
  ...
  # test_sampler=SubsetRandomSampler(test_idx)
    # train_loader = DataLoader(train_set, batch_size=args['train_batch_size'], num_workers=0, shuffle=False,sampler=train_sampler)
  train_loader = DataLoader(train_set, batch_size=2, num_workers=0, shuffle=True,sampler=None)
  test_loader = DataLoader(test_set, batch_size=2, num_workers=0, shuffle=False,sampler=None)
  results=train(net,optimizer,train_loader,test_loader)
  torch.cuda.empty_cache()
  print(results)
      ...
      with autograd.detect_anomaly():
        ...
        # for i, data in enumerate(train_loader):
            if curr_iter == args['lr_step']:
                ...
                curr_iter=0
                net.load_state_dict(torch.load(os.path.join(ckpt_path, 'weights','bestmodel.pth')))
            inputs, labels = data
            ...
            if total_iter % 134==0:
                ...
                if dice>best_dice:
                  ...
                  if not os.path.exists(os.path.join(ckpt_path, 'weights')):
                    # 如果路径不存在，则创建
                    os.makedirs(os.path.join(ckpt_path, 'weights'))
                  torch.save(net.state_dict(), os.path.join(ckpt_path, 'weights', 'bestmodel.pth'))
                  trigger=0
...
def test():
    ...
    Adbs=[]
    Confms=[]
    testing_root=test_path
    gt_path= test_path.replace('image','mask')
    ...
    model.eval()
    with torch.no_grad():
        for file in tqdm(os.listdir(testing_root)):
        # for idx in tqdm(test_idx):
            check_mkdir(os.path.join(ckpt_path, 'prediction'))
            img_name= file
            img = Image.open(os.path.join(testing_root,img_name))
            h,w=img.size
            ...
            pro[pro>=128]=255
            pro[pro<128]=0
            ((Image.fromarray(pro)).resize((h,w),Image.ANTIALIAS)).save(os.path.join(ckpt_path, 'prediction', img_name))
